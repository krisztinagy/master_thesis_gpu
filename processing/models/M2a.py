...
class Vgg19:
    ...
    def inference_graph(self, grayscale, train_mode=None, image_pixels = cfg.image['height']*cfg.image['width'], num_classes = 2):
        ...
        self.conv1_1 = self.first_conv_layer(grayscale_reshaped, 1, 64, "conv1_1_modified", load=False, trainable=True)
        ...
        # 25088 = ((224 // (2 ** 5)) ** 2) * 512
        self.fc6 = self.fc_layer(self.pool5, 25088, 2048, "fc6_modified", load=False, trainable=True)
        self.relu6 = tf.nn.relu(self.fc6)
        
        if train_mode:
            self.relu6 = tf.nn.dropout(self.relu6, self.dropout)

        self.fc7 = self.fc_layer(self.relu6, 2048, 2048, "fc7_modified", load=False, trainable=True)
        self.relu7 = tf.nn.relu(self.fc7)
            
        if train_mode:
            self.relu7 = tf.nn.dropout(self.relu7, self.dropout)

        self.fc8 = self.fc_layer(self.relu7, 2048, num_classes, "fc8_modified", load=False, trainable=True)

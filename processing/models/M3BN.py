...
import sys
sys.path.insert(0, '/home/devop/processing')
...
class Vgg19:
    ...
    def predict(self, grayscale_tensor, train_mode, num_classes):
        ...
        input_nodes = (cfg.image['height'] / 8) * (cfg.image['width'] / 8) * 256
        ...
        tf.summary.histogram('relu6', self.relu6)
        ...
        tf.summary.histogram('relu7', self.relu7)
        
        if train_mode:
            self.relu7 = tf.nn.dropout(self.relu7, self.dropout)

        self.fc8 = self.fc_layer(self.relu7, 1024, 1024, "fc8_modified", load=False, trainable=True)
        self.relu8 = tf.nn.relu(self.fc8)
        tf.summary.histogram('fc8', self.fc8)
        tf.summary.histogram('relu8', self.relu8)
            
        if train_mode:
            self.relu8 = tf.nn.dropout(self.relu8, self.dropout)

        self.fc9 = self.fc_layer(self.relu8, 1024, num_classes, "fc9_modified", load=False, trainable=True)
        tf.summary.histogram('fc9', self.fc9)        
        self.output = tf.nn.softmax(self.fc9, name="prob")
    ...
    def fc_layer(self, bottom, in_size, out_size, name, load, trainable):
        with tf.variable_scope(name):
            ...
            # weights = tf.Variable( np.random.normal( size=(in_size, out_size)).astype(np.float32) )
            ...
            z_hat = (z - mean) / tf.sqrt(var + 0.001)

...
class Vgg19:
    ...
    def inference_graph(self, grayscale, train_mode=None, image_pixels = cfg.image['height']*cfg.image['width'], num_classes = 2):
        ...
        # 25088 = ((224 // (2 ** 5)) ** 2) * 512
        self.fc6 = self.fc_layer(self.pool5, 25088, 4096, "fc6_modified", load=False, trainable=True)
        self.relu6 = tf.nn.relu(self.fc6)
        
        if train_mode:
            self.relu6 = tf.nn.dropout(self.relu6, self.dropout)

        self.fc7 = self.fc_layer(self.relu6, 4096, 4096, "fc7_modified", load=False, trainable=True)

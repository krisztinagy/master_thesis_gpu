...
def run_training(log_dir, last_checkpoint):
    ...
    with treedom_graph.as_default():
        ...
        tf.summary.histogram("Prediction", eval_op_indices)
    ...
    with tf.Session(graph=treedom_graph) as sess:
        ...
        try:
            while not coord.should_stop():
                ...
                #print(correct / FLAGS.batch_size )
                #error_value = np.sum(abs_error)
                error_value = ( FLAGS.batch_size - correct ) / FLAGS.batch_size
                ...
                if iteration % 100 == 0 and not iteration == 0:
                    ...
                    mylog.loss_log(log_dir, label, prediction, iteration, loss_value, error_value, smoothed_loss, smoothed_error )
                    
                    print(label)
                    print(prediction)
                    
                    print("Iteration: %d, Loss: %.6f, Absulute error: %.3f, Smoothed loss: %.6f, Smoothed error: %.3f" % (iteration, loss_value, error_value, smoothed_loss, smoothed_error))
